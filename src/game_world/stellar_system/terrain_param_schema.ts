...
export const TERRAIN_PARAM_SCHEMA: ParamSpec[] = [
    ...
    { path: 'lod.mergePx', group: 'LOD', label: 'Merge (px)', kind: 'baked',
      min: 1, max: 30, step: 0.5, description: 'Merge below this many pixels. Keep < Split (hysteresis; ideally < Split/1.41).' },
    ...
    { path: 'lod.minLevel', group: 'LOD', label: 'Min level', kind: 'baked', int: true,
      min: 0, max: 12, step: 1, description: 'Force-refine the whole sphere to at least this level (rounder far limb).' },
    { path: 'lod.maxLevel', group: 'LOD', label: 'Max level', kind: 'baked', int: true,
      min: 8, max: 32, step: 1, description: 'Hard subdivision cap (df64 cracks well before ~32).' }
];

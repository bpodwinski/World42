...
export const TERRAIN_PARAM_SCHEMA: ParamSpec[] = [
  // --- Relief (FBM noise) -- all baked into the noise header (GPU) and read by CPU collision. -----
  {
    ...
    min: 0, max: 100, step: 0.5, description: 'Overall FBM relief amplitude in km (the inter-crater roughness scale).'
  },
  {
  ...
  },
  {
    ...
    min: 1, max: 32, step: 1, description: 'Number of macro FBM octaves (richness). GPU caps the cascade at 12.'
  },
  {
    path: 'noise.detailOctaves', group: 'Relief', label: 'Detail octaves', kind: 'baked', int: true,
    min: 0, max: 32, step: 1, description: 'Extra near-ground detail octaves that fade in on approach.'
  },
  {
    path: 'noise.lacunarity', group: 'Relief', label: 'Lacunarity', kind: 'baked',
    min: 1.0, max: 3, step: 0.01, description: 'Frequency multiplier per octave (gap between scales).'
  },
  {
    path: 'noise.persistence', group: 'Relief', label: 'Persistence', kind: 'baked',
    min: 0.1, max: 1.0, step: 0.01, description: 'Amplitude falloff per octave. Higher = rougher.'
  },
  {
    path: 'noise.detailRange', group: 'Relief', label: 'Detail range', kind: 'baked',
    min: 1, max: 200, step: 1, description: 'Octave Nyquist lifetime. Higher = keeps fine detail longer (more shimmer).'
  },
  {
    path: 'noise.seed', group: 'Relief', label: 'Seed', kind: 'baked', int: true,
    min: 0, max: 99999, step: 1, description: 'World seed — changes the entire noise + crater field.'
  },
  ...
  // --- Craters -- baked geometry shared GPU (eval + render) and CPU collision. --------------------
  {
    ...
    min: 0, max: 3, step: 0.05, description: 'Global crater depth multiplier (0 = flat, 1 = default).'
  },
  {
    path: 'craters.range', group: 'Craters', label: 'Visibility range', kind: 'baked',
    min: 1, max: 200, step: 1, description: 'Distance factor before a class fades out. Higher = craters stay visible from farther.'
  },
  {
    path: 'craters.near', group: 'Craters', label: 'Near skip', kind: 'baked',
    min: 0, max: 0, step: 0.01, description: 'Per-pixel normal only: skip classes much bigger than the camera distance.'
  },
  {
  ...
  // --- Anti-alias (shading-normal grain control, esp. grazing sun). All baked. --------------------
  {
    ...
    min: 0, max: 64, step: 1, description: 'Diffuse normal antialiasing. Higher = smoother sooner (less grazing-sun grain).'
  },
  {
    path: 'lighting.brdf.meanAaK', group: 'Anti-alias', label: 'Mean-AA correction', kind: 'baked',
    min: 0, max: 1.0, step: 0.02, description: 'Mean-preserving darkening so normal-AA does not over-brighten at grazing (uPerfMask bit5).'
  },
  {
  ...
  // --- TERRAIN LOD / performance (recreates the GPU source on Apply). All baked. ---------------------
  {
    ...
    min: 1, max: 64, step: 0.5, description: 'Split a leaf when its longest edge exceeds this many pixels. LOWER = finer mesh = more cost.'
  },
  {
  ...
  },
  {
    ...
    min: 15, max: 20, step: 1, description: 'GPU leaf-pool capacity = 2^N slots. Raise if the limb under-tessellates (saturation); costs VRAM + topology passes.'
  },
  {
    path: 'lod.minLevel', group: 'LOD', label: 'Min level', kind: 'baked', int: true,
    min: 8, max: 256, step: 1, description: 'Force-refine the whole sphere to at least this level (rounder far limb).'
  },
  {
    path: 'lod.maxLevel', group: 'LOD', label: 'Max level', kind: 'baked', int: true,
    min: 8, max: 256, step: 1, description: 'Hard subdivision cap (df64 cracks well before ~32).'
  }
];

// TERRAIN EvaluateLEB — active-list df64 variant (one thread per ACTIVE slot).
// Identical to terrain_topo_eval_leb_f64 except the slot id is read from
// activeSlots[i] (the bisectorIndices compacted list) instead of the global
// invocation id. Dispatched O(active) instead of O(capacity) via dispatchIndirect
// using the record written by terrain_topo_prepare_eval_leb.
//
...
@group(0) @binding(22) var<storage, read>       activeSlots : array<u32>;
@group(0) @binding(23) var<storage, read>       activeCount : array<u32>;
...
fn main(@builtin(global_invocation_id) gid : vec3<u32>,
    ...
    let heap = heapID[id];
    if (heap_is_zero(heap)) { return; } // safety: compact only writes alive slots

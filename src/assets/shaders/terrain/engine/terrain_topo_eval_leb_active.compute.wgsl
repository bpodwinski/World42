// TERRAIN EvaluateLEB — active-list f32 variant (one thread per ACTIVE slot).
// Identical to terrain_topo_eval_leb except the slot id is read from
// activeSlots[i] (the bisectorIndices compacted list). Dispatched O(active)
// via dispatchIndirect using record 7 written by terrain_topo_prepare_eval_leb.
//

// OCBT memory-pool allocator — GPU core, WGSL. The GPU twin of the CPU oracle
// src/systems/lod/cbt/ocbt/ocbt_cpu_mirror.ts: same decode/reduce semantics, so a
...
// subdivision depth — the whole point versus the implicit 2^D CBT in ../gpu/.
//
...
// cbt_sum_reduction.compute.wgsl). The reference's packed variable-bit-width tree
// (ocbt_256k.hlsl) is a memory/LDS optimization deferred to a later tuning pass; the
// decode RESULTS are identical, so the mirror and the cross-check stay valid.
//
// The includer must define, BEFORE this file:
//     const OCBT_CAPACITY : u32 = <power of two>;
//     const OCBT_DEPTH    : u32 = <log2(OCBT_CAPACITY)>;
// and bind the bitfield at group(0) binding(0), the tree at group(0) binding(1).
// (See ocbt_buffers.ts `poolWgslPreamble` for the generated const lines.)
...
fn pool_freeCount() -> u32 {
    return OCBT_CAPACITY - pool_tree[1];
}
...
fn pool_reduceLeaf(slot : u32) {
    pool_tree[OCBT_CAPACITY + slot] = pool_getBit(slot);
}

// Reduce internal level `level` (0..OCBT_DEPTH-1). Level L has 2^L nodes at absolute
// ids [2^L, 2^(L+1)); `t` is the node index within the level (0..2^L). The driver
// dispatches levels OCBT_DEPTH-1 .. 0 in order, mirroring cbt_sum_reduction.
...
// OcbtPool.decodeBit. Requires handle < pool_count().
fn pool_decodeBit(handle : u32) -> u32 {
    var id = 1u;
    var h = handle;
    for (var d = 0u; d < OCBT_DEPTH; d = d + 1u) {

...
/**
 * Frame Graph task that drives the TERRAIN terrain compute (topology split/merge + EvaluateLEB + draw
 * compaction) inside the graph's execution timeline, immediately before the scene-render task.
 *
 * Why a single wrapper rather than one `FrameGraphComputeShaderTask` per dispatch: the TERRAIN pipeline is
 ...
 * `TerrainScheduler.runCompute()` and just invokes it from the correct point in the frame.
 *
 * ORDERING CONTRACT — do not reorder. The compute writes the TERRAIN STORAGE buffers (heap / positions /
 ...
 */
export class FrameGraphTerrainComputeTask extends FrameGraphTask {
    constructor(
        name: string,
    ...
    public override getClassName(): string {
        return 'FrameGraphTerrainComputeTask';
    }

    public override record(): void {
        const pass = this._frameGraph.addPass(this.name);
        pass.setExecuteFunc((context: FrameGraphContext) => {
            // Nest the per-shader TERRAIN passes under one named region for webgpu-inspector / GPU traces.
            context.pushDebugGroup('TERRAIN compute');
            this.runCompute();

...
import { GPU_FACE_CORNERS, ocbtCorners, ocbtFaceOf, type V3 } from './ocbt_eval_leb';
...
describe('ocbtFaceOf', () => {
    it('maps the 8 root heap ids (8..15) to faces 0..7 at depth 3', () => {
        for (let f = 0; f < 8; f++) expect(ocbtFaceOf(8 + f, 3)).toBe(f);
    });
    it('is convention-invariant (top bits) for deeper ids', () => {
        // heapID 16,17 are children of face 0 (heap 8) at depth 4.
        expect(ocbtFaceOf(16, 4)).toBe(0);
        expect(ocbtFaceOf(17, 4)).toBe(0);
        // heapID 9 -> face 1; its L1 children 18,19 stay on face 1.
        expect(ocbtFaceOf(18, 4)).toBe(1);
        expect(ocbtFaceOf(19, 4)).toBe(1);
    });
});

describe('ocbtCorners (reference convention) — seed', () => {
    it('decodes each root face to its (right, apex, left) seed', () => {
        for (let f = 0; f < 8; f++) {
            const fc = GPU_FACE_CORNERS[f];
            const [v0, v1, v2] = ocbtCorners(8 + f);
...
describe('ocbtCorners — first bisection of face 0', () => {
    ...
    it('the two children share a full edge (watertight sibling split)', () => {
        expect(sharedCorners(ocbtCorners(16), ocbtCorners(17))).toBe(2);
    });
});

describe('ocbtCorners — invariants over a uniform L3 refinement of face 0', () => {
    ...
    it('consecutive sibling leaves share a full edge', () => {
        ...
        for (let h = 64; h < 72; h += 2) {
            expect(sharedCorners(ocbtCorners(h), ocbtCorners(h + 1))).toBe(2);
        }
    });
});

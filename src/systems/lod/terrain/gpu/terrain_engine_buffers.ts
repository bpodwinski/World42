...
export const BINDING = {
    ...
    // Tile cache (Phase 2+)
    SLOT_STATE: 22, // array<u32>  (capacity)         per-slot packed state word
    NORMAL_ATLAS: 23, // storage_texture rg16float   8192×8192  baked normals (XY, Z reconstructed)
    ALBEDO_ATLAS: 24, // storage_texture rgba8unorm   8192×8192  baked albedo + ray brightness
    FREED_TILES: 25, // array<u32>  (1+capacity)      [0]=count, [1..]=freed tile_pool_idx (readback)
    BAKE_WORKLIST: 26 // array<u32>  (1+MAX_BAKE)     [0]=count, [1..]=slot_idx to bake this frame
} as const;
...
/** Maximum tiles baked in a single frame. */
export const MAX_BAKE_PER_FRAME = 64;
/** Texel size of each atlas tile (64×64). */
export const ATLAS_TILE_SIZE = 64;
/** Tiles per row in the 8192×8192 atlas (128). */
export const ATLAS_TILES_PER_ROW = 128;
/** Total tile slots in the atlas (128×128 = 16 384). */
export const ATLAS_TILE_COUNT = ATLAS_TILES_PER_ROW * ATLAS_TILES_PER_ROW;

// Coarse mip pyramid: a second atlas storing an 8×8 downsample of every 64×64 tile
// (factor 8 = "mip level 3"). Generated by the reduce pass after the bake, blended with
// mip0 in the fragment by screen footprint so the cache stays grain-free at distance.
/** Texel size of each coarse tile (8×8 = a ×8 downsample of the 64×64 mip0 tile). */
export const COARSE_TILE_SIZE = 8;
/** Coarse tiles per row — same tile layout as the mip0 atlas (128). */
export const COARSE_TILES_PER_ROW = ATLAS_TILES_PER_ROW;
/** Coarse atlas dimension (128 tiles × 8 = 1024). */
export const COARSE_ATLAS_SIZE = COARSE_TILES_PER_ROW * COARSE_TILE_SIZE;

// Slot-state u32 bit layout:
//   [15:0]  stable_frames   — saturating counter (0 = dead or brand new)
//   [16]    dirty           — 1 = tile needs (re-)baking
//   [17]    has_tile        — 1 = valid tile is in the atlas
//   [31:18] tile_pool_idx   — 14-bit atlas tile index (0..ATLAS_TILE_COUNT-1)
export const SLOT_STABLE_MASK  = 0x0000ffff >>> 0;
export const SLOT_DIRTY_BIT    = 0x00010000 >>> 0;
export const SLOT_HAS_TILE_BIT = 0x00020000 >>> 0;
export const SLOT_TILE_SHIFT   = 18;
export const SLOT_TILE_MASK    = 0x3fff >>> 0;
/** Frames a slot must be stable before its tile is baked. */
export const SLOT_BAKE_THRESHOLD = 4;

export function slotStateWords(capacity: number): number {
    assertPowerOfTwo(capacity);
    return capacity;
}
export function freedTilesWords(capacity: number): number {
    assertPowerOfTwo(capacity);
    return 1 + capacity;
}
export function bakeWorklistWords(): number {
    return 1 + MAX_BAKE_PER_FRAME;
}
...
/** WGSL const preamble for the tile-cache slot-state layout. */
export function slotStateWgslPreamble(): string {
    return (
        `const SLOT_STABLE_MASK    : u32 = ${SLOT_STABLE_MASK}u;\n` +
        `const SLOT_DIRTY_BIT      : u32 = ${SLOT_DIRTY_BIT}u;\n` +
        `const SLOT_HAS_TILE_BIT   : u32 = ${SLOT_HAS_TILE_BIT}u;\n` +
        `const SLOT_TILE_SHIFT     : u32 = ${SLOT_TILE_SHIFT}u;\n` +
        `const SLOT_TILE_MASK      : u32 = ${SLOT_TILE_MASK}u;\n` +
        `const SLOT_BAKE_THRESHOLD : u32 = ${SLOT_BAKE_THRESHOLD}u;\n` +
        `const MAX_BAKE_PER_FRAME  : u32 = ${MAX_BAKE_PER_FRAME}u;\n` +
        `const ATLAS_TILE_SIZE     : u32 = ${ATLAS_TILE_SIZE}u;\n` +
        `const ATLAS_TILES_PER_ROW : u32 = ${ATLAS_TILES_PER_ROW}u;\n` +
        `const ATLAS_TILE_COUNT    : u32 = ${ATLAS_TILE_COUNT}u;\n` +
        `const COARSE_TILE_SIZE     : u32 = ${COARSE_TILE_SIZE}u;\n` +
        `const COARSE_TILES_PER_ROW : u32 = ${COARSE_TILES_PER_ROW}u;\n`
    );
}

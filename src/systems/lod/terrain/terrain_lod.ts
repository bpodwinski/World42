/**
 * TERRAIN LOD / performance knobs. Drive tessellation density and the GPU pool size. Formerly hardcoded
 * in TerrainPlanet.createSource; now a per-profile block (planet_profiles.ts) so the options menu can
 * tune them. Changing any of these recreates the TERRAIN source (new pool buffers + thresholds), so the
 * topology reconverges. Pure data — no Babylon import, so the config layer can depend on it freely.
 */

export type TerrainLodParams = {
...
/** Default TERRAIN LOD knobs (the values previously hardcoded in createSource). */
export const DEFAULT_LOD: TerrainLodParams = {
    splitPx: 16,
    mergePx: 8,

...
`TERRAIN_AMBIENT` (constante bakée) remplacée par `uAmbient` (uniform runtime). `uLightIntensity` ajouté et câblé depuis `TerrainPlanetOptions.starIntensity` → `TerrainSource` → `setLightIntensity()` per-frame.

**Fichiers modifiés :**
- `src/systems/lod/terrain/gpu/terrain_render_material.ts`
- `src/systems/lod/terrain/gpu/terrain_source.ts`
- `src/systems/lod/terrain/terrain_scheduler.ts`

---

### Axe C — AO de courbure procédural ✅

`TERRAIN_AO_STRENGTH = 0.35`. `ao = 1.0 - TERRAIN_AO_STRENGTH * clamp(1 - dot(nSlope, dir), 0, 1)`. Appliqué à `uAmbient` uniquement. Gated `uPerfMask` bit3. Basé sur `nSlope` (lissé à `TERRAIN_SLOPE_DIST` km) pour un AO macro-forme (cratères/vallées), sans speckle micro-bump.

**Fichier modifié :** `src/systems/lod/terrain/gpu/terrain_render_material.ts`

---

### Axe D — Spéculaire GGX (terme D seul) ✅ → remplacé par Cook-Torrance complet (Round 2)

Premier GGX avec terme D uniquement et `TERRAIN_SPEC_STRENGTH`. Remplacé en Round 2 (axe E) par le modèle complet D·F·G.
...
- **Fresnel Schlick** : `F = TERRAIN_F0 + (1−TERRAIN_F0)·(1−VdH)⁵`, avec `TERRAIN_F0 = 0.04` (roche diélectrique)
- **Smith-GGX geometry** : `G = G1(NdL)·G1(NdV)`, `k = alpha/2` (approximation IBL)
- Résultat : `spec = (D·F·G)/(4·NdL·NdV+ε)·NdL`

`TERRAIN_SPEC_STRENGTH` supprimé. `TERRAIN_F0` le remplace (physiquement fondé). Gated bit4.
...
`fogFactor = exp(-uAtmoDensity · camDistKm · altFactor)` où `altFactor = clamp(1 - altKm/(TERRAIN_RADIUS·0.01), 0, 1)`.  
...
Setters : `setAtmoDensity()` / `setAtmoColor()` sur `TerrainRenderMaterial`.
...
**Fichier modifié :** `src/systems/lod/terrain/gpu/terrain_source.ts`
...
- `stellar_catalog_loader.ts` : lit `body.lighting` depuis `BodyJSON`, passe à `resolveLighting()`, propage via `LoadedBody` → `TerrainPlanet` → `TerrainSource` → `buildTerrainRenderMaterial()`
...
**Non configurables (intentionnel) :** `TERRAIN_AA_FOOTPRINT_KM = 0.03` (réglage d'antialiasing), `TERRAIN_GROUND_BASE_FREQ = radius * 1000` (fréquence physique, 1 m de base)
...
- `src/systems/lod/terrain/gpu/terrain_render_material.ts` — `bakedHeader()` lit depuis `opts.lighting`
- `src/systems/lod/terrain/gpu/terrain_source.ts` — `TerrainSourceOptions.lighting`, extraction albedo/ambient/atmo
- `src/systems/lod/terrain/terrain_scheduler.ts` — `TerrainPlanetOptions.lighting`, transmission vers `TerrainSource`
...
**Fichiers à toucher :** `terrain_render_material.ts` (option B/C), nouveau render pass WebGPU (option A).
...
**Fichiers à toucher :** `terrain_render_material.ts`, `terrain_source.ts`, `stellar_catalog_loader.ts`.
...
- `nSlope` (landform normal à `TERRAIN_SLOPE_DIST` km) est utilisé pour l'AO et le slope-splatting. Ne pas le remplacer par `nLocal` (micro-bumps → speckle).
